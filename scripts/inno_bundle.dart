...
void main() {
  ...
  if (!pubspecFile.existsSync()) {
    stderr.writeln('❌ Error: pubspec.yaml not found in $projectRoot');
    exit(1);
  }
  ...
  if (rawName == null || versionRaw == null) {
    stderr.writeln('❌ Error: Name or Version missing in pubspec.yaml');
    exit(1);
  }
  ...
  if (appId == null) {
    stderr.writeln('❌ Error: inno_bundle.id is missing! Required for Windows identification.');
    exit(1);
  }
  ...
  if (!Directory(buildDir).existsSync()) {
    stderr.writeln('❌ Error: Build artifacts not found at: $buildDir');
...
''';
  ...
  print('✔ Generated Inno Setup script at: $issPath');
  ...
  if (!File(isccPath).existsSync()) {
    stderr.writeln('❌ Error: Inno Setup Compiler (ISCC.exe) not found at $isccPath');
    exit(1);
  }
  ...
  print('✔ Success! Your installer is ready in: $outputDir');
}